    ...
    try:
        ...
        if cloudtype == 'moodle':
            ...
            if loged:
                ...
                bot.editMessageText(message,'No eh logrado conectar con la pagina')
...
def onmessage(update,bot:ObigramClient):
    try:
        ...
        if username == tl_admin_user or user_info:  # validate user
            ...
            mensaje = "Usted no tiene acceso.\nPor favor Contacta con mi Programador @AlexeyGarciaDias\n"
        ...
        if '/info' in msgText:
            ...
            if isadmin:
                ...
                bot.sendMessage(update.message.chat.id,'No Puedo mostrarle esta información')    
            return
        ...
        if '/uptype' in msgText:
            try:
                ...
                bot.sendMessage(update.message.chat.id,'Error en el comando /uptype (typo de subida (evidence,draft,calendario))')
            return
        if '/proxy' in msgText:
            try:
                cmd = str(msgText).split(' ',2)
        ...
        if '/cancel_' in msgText:
            try:
                ...
                time.sleep(3)
                bot.editMessageText(msg,'Tarea Cancelada con Exito')
            except Exception as ex:
                print(str(ex))
            return
        #end
        ...
        if '/start' in msgText:
            ...
            start_msg+= ' Soporte @AlexeyGarciaDias \n'
            start_msg+= ' Antes de comenzar vea el /tutorial \n'
            ...
            if loged:
                ...
                client.logout()
                bot.editMessageText(message,'Archivo Borrado Correctamente')
            else:
                bot.editMessageText(message,'Error y Causas \n1-Revise su Cuenta\n2-Servidor Desabilitado: '+client.path)
...
def main():
    ...
    bot_token = '5248595768:AAErdC87a0Se20Ss2bnukeNgp7pEHs1VZS4'
